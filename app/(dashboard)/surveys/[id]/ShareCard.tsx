...
export default function ShareCard({ surveyId }: { surveyId: string }) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      // ✅ FIXED: Now points to /assessment/
  ...
  return (
    <div className="bg-white text-black p-8 rounded-[2.5rem] shadow-2xl flex flex-col items-center text-center h-full">
      ...
      <div ref={qrRef} className="bg-gray-50 p-5 rounded-[2rem] border border-gray-100 mb-6">
        {shareUrl ? <QRCode value={shareUrl} size={160} level="H" /> : <div className="w-[160px] h-[160px] bg-gray-100 animate-pulse rounded-xl" />}
      </div>
      <div className="w-full space-y-3">
        <button onClick={copyToClipboard} className={`w-full py-4 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] transition-all active:scale-[0.98] ${copied ? "bg-green-600 text-white" : "bg-black text-white hover:bg-gray-900"}`}>
          {copied ? "Link Copied" : "Copy Portal Link"}
        </button>
        <button onClick={downloadQRCode} className="w-full py-4 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] border-2 border-black text-black hover:bg-gray-50 transition-all active:scale-[0.98]">
          Download QR Image
        </button>
      </div>
    </div>
