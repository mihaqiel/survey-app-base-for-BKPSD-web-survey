...
export default function GlobalExportButton() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      data.forEach((svc: (typeof data)[number]) => {
        svc.rawResponses.forEach((r: (typeof svc.rawResponses)[number]) => {
          const row = wsData.addRow([
            ...
            svc.serviceName, r.Nama, r.Pegawai, r.Tanggal, r.JK, r.Pendidikan, r.Umur, r.Pekerjaan,
            r.Difabel ?? "Tidak", r.JenisDisabilitas ?? "-",
      ...
      const summaryRows = [
        ["Rerata IKM Per Unsur", "", { formula: `IFERROR(AVERAGE(C2:C${lastDataRow}),0)` }, { formula: `IFERROR(AVERAGE(D2:D${lastDataRow}),0)` }, { formula: `IFERROR(AVERAGE(E2:E${lastDataRow}),0)` }, { formula: `IFERROR(AVERAGE(F2:F${lastDataRow}),0)` }, { formula: `IFERROR(AVERAGE(G2:G${lastDataRow}),0)` }, { formula: `IFERROR(AVERAGE(H2:H${lastDataRow}),0)` }, { formula: `IFERROR(AVERAGE(I2:I${lastDataRow}),0)` }, { formula: `IFERROR(AVERAGE(J2:J${lastDataRow}),0)` }, { formula: `IFERROR(AVERAGE(K2:K${lastDataRow}),0)` }, { formula: `IFERROR(AVERAGE(L2:L${lastDataRow}),0)` }, "", ""],
  ...
  return (
    <button
      ...
      disabled={loading}
      className="btn-shimmer group flex items-center gap-2 px-5 py-2.5 text-white text-[10px] font-black uppercase tracking-widest transition-all duration-200 hover:scale-[1.03] hover:shadow-[0_6px_20px_rgba(0,156,197,0.35)] active:scale-[0.97] disabled:opacity-50 disabled:hover:scale-100 disabled:hover:shadow-none"
