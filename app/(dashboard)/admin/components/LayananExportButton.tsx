...
interface Response {
  id: string; nama: string; tglLayanan?: Date | string | null; createdAt: Date | string;
  jenisKelamin: string; pendidikan: string; usia: number; pekerjaan: string;
  isDifabel?: string | null; jenisDisabilitas?: string | null;
  u1: number; u2: number; u3: number; u4: number; u5: number;
  u6: number; u7: number; u8: number; u9: number;
  saran?: string | null; pegawai?: { nama: string } | null;
}

interface Props {
  layananNama: string; periodLabel: string; responses: Response[];
}
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      wsData.columns = [
        ...
        { header: "Persyaratan", key: "u1", width: 14 }, { header: "Prosedur",     key: "u2", width: 12 },
        { header: "Jangka Waktu",key: "u3", width: 14 }, { header: "Tarif",        key: "u4", width: 10 },
        { header: "Produk",      key: "u5", width: 12 }, { header: "Kompetensi",   key: "u6", width: 14 },
        { header: "Perilaku",    key: "u7", width: 12 }, { header: "Pengaduan",    key: "u8", width: 12 },
        { header: "Sarpras",     key: "u9", width: 12 }, { header: "Saran",        key: "saran", width: 35 },
      ];
      ...
      responses.forEach((r, idx) => {
        const tgl = r.tglLayanan ? new Date(r.tglLayanan).toLocaleDateString("id-ID") : new Date(r.createdAt).toLocaleDateString("id-ID");
        const row = wsData.addRow([
          "Badan Kepegawaian dan Pengembangan Sumber Daya Manusia", layananNama,
          r.nama, r.pegawai?.nama ?? "-", tgl, r.jenisKelamin, r.pendidikan, r.usia, r.pekerjaan,
        ...
        row.eachCell(cell => {
          ...
          cell.border = { top: { style: "hair", color: { argb: "FFBDC3C7" } }, bottom: { style: "hair", color: { argb: "FFBDC3C7" } }, left: { style: "hair", color: { argb: "FFBDC3C7" } }, right: { style: "hair", color: { argb: "FFBDC3C7" } } };
        });
      });
      ...
      wsRekap.columns = [
        { key: "a", width: 35 }, { key: "b", width: 16 }, { key: "c", width: 13 }, { key: "d", width: 13 },
        { key: "e", width: 13 }, { key: "f", width: 10 }, { key: "g", width: 13 }, { key: "h", width: 13 },
        { key: "i", width: 12 }, { key: "j", width: 12 }, { key: "k", width: 12 }, { key: "l", width: 22 },
        { key: "m", width: 10 }, { key: "n", width: 16 },
      ];

      const rekapHeader = wsRekap.addRow([
        "Jenis Layanan", "Total Responden", "Persyaratan", "Prosedur", "Jangka Waktu", "Tarif", "Produk",
        "Kompetensi", "Perilaku", "Pengaduan", "Sarpras", "IKM Per Jenis Layanan", "MUTU", "PREDIKAT",
      ]);
      ...
      if (responses.length > 0) {
        ...
        dataRow.eachCell(cell => {
          ...
          cell.font = { size: 10, name: "Arial" }; cell.alignment = { horizontal: "center" }; cell.numFmt = '#,##0.00';
          cell.border = { top: { style: "hair", color: { argb: "FFBDC3C7" } }, bottom: { style: "hair", color: { argb: "FFBDC3C7" } }, left: { style: "hair", color: { argb: "FFBDC3C7" } }, right: { style: "hair", color: { argb: "FFBDC3C7" } } };
        });
      ...
      summaryDefs.forEach(rowData => {
        ...
        sumRow.eachCell(cell => {
          ...
          cell.alignment = { horizontal: "center" }; cell.numFmt = '#,##0.00';
