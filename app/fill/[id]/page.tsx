...
export default async function FillSurveyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 🎯 If the node is closed, this "Virtual Wall" renders instead of the redirect
  if (isClosed) {
    return (
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 text-white min-h-screen">
      <div className="mb-12 text-center flex flex-col items-center gap-4">
        <h1 className="text-4xl font-black uppercase italic tracking-tighter leading-none">{survey.title}</h1>
      ...
      <form action={submitResponse} className="space-y-8">
        ...
        {survey.questions.map((q: any, index: number) => (
          <div key={q.id} className="bg-gray-900/40 p-10 rounded-[2.5rem] border border-white/5 shadow-2xl transition-all hover:border-blue-500/10 group text-left">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
                {RATING_OPTIONS.map((opt) => (
                  <label key={opt.value} className="cursor-pointer group/item">
                    ...
                    <div className={`py-6 text-center rounded-[1.5rem] border border-white/5 bg-white/[0.03] transition-all duration-300 peer-checked:text-white peer-checked:scale-[1.05] hover:bg-white/[0.08] ${opt.activeClass}`}>
                      <span className="block font-black text-3xl mb-1 tracking-tighter italic">{opt.value}</span>
                      <span className="text-[8px] uppercase font-black opacity-30 peer-checked:opacity-100 tracking-widest transition-opacity">{opt.label}</span>
                    </div>
                  </label>
                ))}
              </div>
            ) : (
              <textarea name={`answer_${q.id}`} required placeholder="Provide regional feedback..." className="w-full p-8 bg-white/[0.03] border border-white/5 rounded-[2rem] outline-none focus:border-blue-500/50 focus:bg-white/[0.05] transition-all h-40 text-sm font-medium" />
            )}
          </div>
        ))}
