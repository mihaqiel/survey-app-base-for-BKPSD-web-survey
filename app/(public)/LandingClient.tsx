...
export default function LandingClient({ surveyToken }: { surveyToken: string }) {
  ...
  return (
    <div className="text-[#132B4F] font-sans flex flex-col">
        ...
        <div className="relative max-w-5xl mx-auto px-6 py-20 md:py-28">
          <div className="max-w-3xl">
            <div className="animate-fade-up delay-75 inline-flex items-center gap-2 bg-white/10 border border-white/20 px-4 py-2 mb-8">
              ...
              <span className="text-[9px] font-black uppercase tracking-[0.3em] text-white/70">Portal Resmi — BKPSDM</span>
            </div>
            <h1 className="animate-fade-up delay-150 text-5xl md:text-6xl font-black uppercase tracking-tight text-white leading-[0.9] mb-6">
              Survei<br /><span className="text-[#f3f2ef]">Kepuasan</span><br />Masyarakat
            </h1>
      ...
      <section ref={statsRef} className="scroll-fade bg-[#F0F4F8] border-b border-gray-200">
        <div className="max-w-5xl mx-auto px-6 py-6 grid grid-cols-1 sm:grid-cols-3 sm:divide-x divide-gray-300 gap-4 sm:gap-0">
          {[
            ...
            <div key={item.label} className={`px-8 first:pl-0 last:pr-0 animate-count-up ${item.delay}`}>
              <p className="text-3xl font-black text-[#132B4F]">{item.val}</p>
              <p className="text-[10px] font-black uppercase tracking-widest text-[#009CC5] mt-0.5">{item.label}</p>
      ...
      {/* FEATURES */}
      <section ref={featRef} className="scroll-fade max-w-5xl mx-auto px-6 py-16 w-full">
        <div className="flex items-center gap-3 mb-10">
          <div className="w-1 h-8 bg-[#009CC5]" />
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-[#009CC5]">Tentang Sistem</p>
            <h2 className="text-2xl font-black uppercase tracking-tight text-[#132B4F]">Fitur Utama SKM</h2>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-gray-200">
          {[
            { icon: <BarChart3 className="w-5 h-5" />, title: "Real-time IKM", desc: "Perhitungan Indeks Kepuasan Masyarakat secara otomatis dan akurat berdasarkan setiap respons yang masuk.", accent: "#009CC5", delay: "delay-75" },
            { icon: <ShieldCheck className="w-5 h-5" />, title: "Sesuai Regulasi", desc: "Menggunakan 9 unsur pelayanan standar Kementerian PANRB sesuai Permenpan RB Nomor 14 Tahun 2017.", accent: "#FAE705", delay: "delay-150" },
            { icon: <QrCode className="w-5 h-5" />, title: "Akses QR Code", desc: "Kemudahan akses survei bagi responden melalui pemindaian QR Code yang tersedia di loket pelayanan.", accent: "#132B4F", delay: "delay-225" },
          ].map((f, i) => (
            <div key={f.title}
              className={`card-hover p-8 ${i > 0 ? "border-l border-gray-200" : ""} hover:bg-[#F0F4F8] transition-colors animate-fade-up ${f.delay}`}>
              <div className="w-10 h-10 flex items-center justify-center mb-5" style={{ backgroundColor: f.accent + "20", color: f.accent }}>
                {f.icon}
              </div>
              <div className="w-8 h-0.5 mb-4 animate-draw-line" style={{ backgroundColor: f.accent }} />
              <h3 className="text-sm font-black uppercase tracking-widest text-[#132B4F] mb-3">{f.title}</h3>
      ...
      {/* HOW TO */}
      <section ref={howtoRef} className="scroll-fade bg-[#F0F4F8] border-y border-gray-200">
        <div className="max-w-5xl mx-auto px-6 py-16 w-full">
          <div className="flex items-center gap-3 mb-10">
            <div className="w-1 h-8 bg-[#FAE705]" />
            <div>
              <p className="text-[9px] font-black uppercase tracking-[0.3em] text-[#009CC5]">Panduan</p>
              <h2 className="text-2xl font-black uppercase tracking-tight text-[#132B4F]">Cara Pengisian Survei</h2>
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-px bg-gray-200">
            {[
              { step: "01", icon: <Smartphone className="w-6 h-6 text-[#009CC5]" />, title: "Pindai QR Code", desc: "Temukan QR Code di loket pelayanan BKPSDM lalu pindai menggunakan kamera HP." },
              { step: "02", icon: <KeyRound className="w-6 h-6 text-[#009CC5]" />, title: "Masukkan Token", desc: "Masukkan kode token akses yang tertera pada QR Code untuk membuka portal survei." },
              { step: "03", icon: <ClipboardList className="w-6 h-6 text-[#009CC5]" />, title: "Isi Formulir", desc: "Isi data diri, pilih pegawai, beri rating, dan berikan penilaian terhadap 9 unsur kepuasan." },
              { step: "04", icon: <CheckCircle2 className="w-6 h-6 text-[#009CC5]" />, title: "Kirim Survei", desc: "Klik tombol kirim. Data Anda akan langsung tersimpan dan diolah secara otomatis." },
            ].map((item, i) => (
              <div key={item.step} className="bg-white p-7 group card-hover animate-fade-up" style={{ animationDelay: `${i * 75}ms` }}>
                <div className="flex items-start justify-between mb-5">
                  <div className="transition-transform duration-200 group-hover:scale-110">{item.icon}</div>
                  <span className="text-4xl font-black text-gray-100 group-hover:text-[#009CC5]/20 transition-colors">{item.step}</span>
                </div>
                <h3 className="text-sm font-black uppercase tracking-widest text-[#132B4F] mb-2">{item.title}</h3>
      ...
      {/* CTA */}
      <section ref={ctaRef} className="scroll-fade max-w-5xl mx-auto px-6 py-16 w-full">
        <div className="bg-[#132B4F] overflow-hidden relative">
          ...
          <div className="relative px-10 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="animate-slide-left">
              <p className="text-[9px] font-black uppercase tracking-[0.3em] text-[#009CC5] mb-1">Mulai Sekarang</p>
              <h3 className="text-xl font-black uppercase tracking-tight text-white">Bagikan Pendapat Anda</h3>
              <p className="text-sm text-white/50 font-medium mt-1">Survei ini bersifat rahasia dan membantu kami meningkatkan layanan.</p>
            </div>
            <Link href={surveyHref}
