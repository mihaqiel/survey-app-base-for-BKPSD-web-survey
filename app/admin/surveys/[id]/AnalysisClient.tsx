...
export function AnalysisClient({ survey, surveyId }: { survey: any, surveyId: string }) {
  ...
  const getRatingBadge = (value: string) => {
    ...
    const tiers = [
      ...
      { label: "VERY GOOD", color: "bg-cyan-900/40 text-cyan-400 border-orange-700/30" },
      { label: "EXCELLENT", color: "bg-green-900/40 text-green-400 border-green-700/30" }
    ];

    const tier = tiers[cleanDigit - 1]; 
    return <span className={`px-2 py-1 rounded border text-[9px] font-black uppercase tracking-widest ${tier.color}`}>{tier.label}</span>;
  };
  ...
  const downloadQRCode = () => {
    ...
    img.onload = () => {
      ...
      if (ctx) {
        ...
        downloadLink.href = pngUrl; downloadLink.download = `QR_${survey.title.replace(/\s+/g, "_")}.png`;
  ...
  return (
    <div className="p-10 text-white min-h-screen max-w-7xl mx-auto text-left">
      <Link href="/admin" className="text-gray-500 hover:text-white mb-8 inline-block transition-colors font-black uppercase text-[10px] tracking-widest">← Back to Console</Link>
      <div className="grid lg:grid-cols-3 gap-8 mb-12">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center gap-4">
            <h1 className="text-5xl font-black italic uppercase tracking-tighter leading-none">{survey.title}</h1>
            <div className={`px-4 py-1 rounded-full border text-[10px] font-black tracking-widest ${badgeStatus.color}`}>• {badgeStatus.label}</div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-gray-900/50 border border-gray-800 p-6 rounded-2xl">
              <p className="text-[9px] font-black uppercase text-gray-500 mb-2 italic">Reporting Window</p>
            ...
            <div className="bg-gray-900/50 border border-gray-800 p-6 rounded-2xl text-center">
              <p className="text-[9px] font-black uppercase text-gray-500 mb-2 italic">Latest Mean</p>
              <span className="text-xl font-black italic">{meanScore.toFixed(2)}</span>
            </div>
            <div className="bg-gray-900/50 border border-gray-800 p-6 rounded-2xl text-center">
              <p className="text-[9px] font-black uppercase text-gray-500 mb-2 italic">Latest Index</p>
        ...
        <div className="bg-white p-6 rounded-[2.5rem] flex flex-col items-center justify-between shadow-2xl">
           <div className="w-full text-center mb-4">
              <p className="text-[9px] font-black uppercase tracking-[0.3em] text-gray-400 mb-1 italic">Access Portal</p>
           ...
           <div className="w-full space-y-2">
             <button onClick={handleCopy} className={`w-full py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all ${copied ? 'bg-green-500 text-white' : 'bg-black text-white hover:bg-gray-900'}`}>{copied ? 'Link Copied!' : 'Copy Portal Link'}</button>
             <button onClick={downloadQRCode} className="w-full py-3 rounded-2xl font-black uppercase text-[9px] tracking-widest border-2 border-black text-black hover:bg-gray-50 transition-all">Download QR</button>
           </div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-black/20 text-[9px] font-black uppercase tracking-widest text-gray-600">
                ...
                {survey.questions.map((q: any) => (<th key={q.id} onClick={() => setSelectedQuestionId(q.id)} className={`p-6 border-b border-gray-800 cursor-pointer hover:text-blue-400 ${selectedQuestionId === q.id ? 'text-blue-500 bg-blue-500/5' : ''}`}>{q.text}</th>))}
              </tr>
            </thead>
              ...
              {survey.responses.map((resp: any) => (
                <tr key={resp.id} className="hover:bg-white/[0.02]">
                  ...
                  {survey.questions.map((q: any) => {
                    ...
                    return <td key={q.id} onClick={() => setSelectedQuestionId(q.id)} className={`p-6 text-sm cursor-pointer ${selectedQuestionId === q.id ? 'bg-blue-500/5' : ''}`}>{q.type === "SCORE" ? getRatingBadge(val) : <span className="text-gray-400">{val}</span>}</td>;
                  })}
                </tr>
              ))}
            </tbody>
      ...
      {selectedQuestionId && (
        <div className="mt-8 p-10 bg-blue-600/5 border border-blue-500/20 rounded-[2.5rem] relative">
          ...
          <div className="grid md:grid-cols-2 gap-12">
            <div className="space-y-6">
              <h2 className="text-3xl font-black italic uppercase tracking-tighter">{survey.questions.find((q: any) => q.id === selectedQuestionId)?.text}</h2>
